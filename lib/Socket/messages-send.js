...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendLatex: async (jid, quoted, options) => {
            if (quoted && !quoted.key && (quoted.expressions || quoted.text)) {
                options = quoted
                quoted = undefined
            }
         ...
         * @param {object}   quoted
         * @param {object}   options          - { text?, expressions, headerText?, footer? }
         * @param {Function} renderLatexToPng - async (expr) => { buffer, width, height }
        ...
        sendLatexImage: async (jid, quoted, options, renderLatexToPng, uploadFn) => {
            ...
            if (!uploadFn) {
                uploadFn = async (buffer) => {
                    const media = await Utils_1.prepareWAMessageMedia(
                        { image: buffer },
                        { upload: waUploadToServer, logger }
                    )
                    return {
                        url: media.imageMessage?.url,
                        directPath: media.imageMessage?.directPath
                    }
                }
            }
