...
const makeMessagesSocket = (config) => {
    ...
    const sock = {
        ...
        /**
         * Render LaTeX to PNG image using online codecogs API and send it.
         * @param {string} jid
         * @param {object} [quoted]
         * @param {object|string} options - LaTeX string or options object { text?, expressions?, caption? }
         */
        sendLatexImage: async (jid, quoted, options) => {
            ...
            const integrationFormula = formulas[0]
            let dpiPrefix = '\\dpi{600}'
            let actualFormula = integrationFormula
            if (integrationFormula.startsWith('\\dpi')) {
                const match = integrationFormula.match(/^\\dpi\{\d+\}/)
                if (match) {
                    dpiPrefix = match[0]
                    actualFormula = integrationFormula.substring(dpiPrefix.length)
                }
            }
            const dpiFormula = `${dpiPrefix}\\colorbox{white}{${actualFormula}}`
            const mediaurl = `https://latex.codecogs.com/png.image?${encodeURIComponent(dpiFormula)}`
            ...
            if (options && typeof options === 'object') {
                ...
                if (captionText) {
                    msgContent.caption = captionText
                }
            }
        ...
        /**
         * Render multiple LaTeX expressions as separate online rendered images.
         * @param {string} jid
         * @param {object} [quoted]
         * @param {object|string} options - LaTeX string or options object { text?, expressions?, caption? }
         */
        sendLatexInlineImage: async (jid, quoted, options) => {
            if (quoted && !quoted.key && (quoted.expressions || quoted.text || typeof quoted === 'string' || (Array.isArray(quoted) && quoted.length > 0))) {
                options = quoted
                quoted = undefined
            }
            ...
            const albumItems = formulas.map((formula, index) => {
                let dpiPrefix = '\\dpi{600}'
                let actualFormula = formula
                if (formula.startsWith('\\dpi')) {
                    const match = formula.match(/^\\dpi\{\d+\}/)
                    if (match) {
                        dpiPrefix = match[0]
                        actualFormula = formula.substring(dpiPrefix.length)
                    }
                }
                const dpiFormula = `${dpiPrefix}\\colorbox{white}{${actualFormula}}`
                const mediaurl = `https://latex.codecogs.com/png.image?${encodeURIComponent(dpiFormula)}`
                ...
                if (options && typeof options === 'object') {
                    if (options.caption === true) {
                        item.caption = formula
                    } else if (index === 0 && (options.caption || options.text)) {
                        item.caption = options.caption || options.text
                    }
                }
                return item
            })
